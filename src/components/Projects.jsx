// src/components/Projects.jsx
import React, { useState, useEffect, useRef } from "react";
import { Link } from "react-router-dom";
import { projects } from "../data/projects";
import {
  FaGamepad,
  FaLaptop,
  FaPalette,
  FaCube,
  FaGithub,
  FaExternalLinkAlt,
  FaCalendarAlt,
  FaCodeBranch,
  FaStar,
  FaCode,
} from "react-icons/fa";
import { motion, AnimatePresence } from "framer-motion";
import { FaGear, FaHardDrive } from "react-icons/fa6";

/* ── GitHub repo data cache ── */
const repoCache = {};

async function fetchRepoData(githubUrl) {
  if (!githubUrl) return null;
  const match = githubUrl.match(/github\.com\/([^/]+\/[^/]+)/);
  if (!match) return null;
  const repo = match[1];
  if (repoCache[repo] !== undefined) return repoCache[repo];
  try {
    const res = await fetch(`https://api.github.com/repos/${repo}`);
    if (!res.ok) {
      repoCache[repo] = null;
      return null;
    }
    const data = await res.json();
    repoCache[repo] = data;
    return data;
  } catch {
    repoCache[repo] = null;
    return null;
  }
}

/* ── Variants ── */
const containerVariants = {
  hidden: { opacity: 0 },
  show: { opacity: 1, transition: { staggerChildren: 0.1 } },
};
const fadeUp = {
  hidden: { opacity: 0, y: 24 },
  show: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } },
};

/* ── GitHub stats mini strip ── */
function GitHubStrip({ githubUrl }) {
  const [data, setData] = useState(undefined);
  useEffect(() => {
    fetchRepoData(githubUrl).then(setData);
  }, [githubUrl]);
  if (!githubUrl) return null;
  if (data === undefined)
    return (
      <div className="project-github-stats">
        <div className="github-loading">
          <div className="github-loading-dot" />
          <div className="github-loading-dot" />
          <div className="github-loading-dot" />
          <span>Loading repo stats…</span>
        </div>
      </div>
    );
  if (!data) return null;
  return (
    <div className="project-github-stats">
      <div className="github-stat">
        <FaStar />
        <span>{data.stargazers_count?.toLocaleString() ?? 0}</span>
      </div>
      <div className="github-stat">
        <FaCode />

        <span>{data.forks_count?.toLocaleString() ?? 0}</span>
      </div>
      {data.language && (
        <div className="github-stat">
          <span
            style={{
              width: 8,
              height: 8,
              borderRadius: "50%",
              background: "var(--accent)",
              display: "inline-block",
            }}
          />
          <span>{data.language}</span>
        </div>
      )}
    </div>
  );
}

/* ── Icon picker ── */
function getProjectIcon(tags) {
  if (tags.some((t) => t.name === "Game Design"))
    return <FaGamepad className="project-main-icon" />;
  if (tags.some((t) => t.name === "Plugin"))
    return <FaGear className="project-main-icon" />;
  if (tags.some((t) => t.name === "Arduino"))
    return <FaHardDrive className="project-main-icon" />;
  if (tags.some((t) => t.name === "Graphics"))
    return <FaPalette className="project-main-icon" />;
  if (tags.some((t) => t.name === "Website"))
    return <FaLaptop className="project-main-icon" />;
  return <FaCube className="project-main-icon" />;
}

const Projects = () => {
  const [filter, setFilter] = useState("all");

  const filteredProjects =
    filter === "all"
      ? projects
      : projects.filter((p) =>
          p.tags.some((t) => t.name.toLowerCase() === filter)
        );

  const allCategories = [
    "all",
    ...new Set(
      projects.flatMap((p) => p.tags.map((t) => t.name.toLowerCase()))
    ),
  ];

  return (
    <section id="projects" className="section">
      <motion.div
        className="container"
        variants={containerVariants}
        initial="hidden"
        whileInView="show"
        viewport={{ once: true, amount: 0.1 }}
      >
        <div className="section-title">
          <div className="section-label">Work</div>
          <motion.h2 variants={fadeUp}>
            My <em>Projects</em>
          </motion.h2>
          <p>A collection of games, tools, and websites I've created.</p>
        </div>

        {/* Filters */}
        <motion.div className="projects-filters" variants={containerVariants}>
          {allCategories.map((cat) => (
            <motion.button
              key={cat}
              className={`filter-btn ${filter === cat ? "active" : ""}`}
              onClick={() => setFilter(cat)}
              whileHover={{ scale: 1.04 }}
              whileTap={{ scale: 0.96 }}
            >
              {cat.charAt(0).toUpperCase() + cat.slice(1)}
            </motion.button>
          ))}
        </motion.div>

        {/* Grid */}
        <motion.div
          className="projects-grid"
          variants={containerVariants}
          layout
        >
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, idx) => (
              <motion.div
                key={project.id}
                className="project-card"
                variants={fadeUp}
                layout
                exit={{ opacity: 0, scale: 0.95 }}
                whileHover={{ y: -6 }}
              >
                {/* Thumbnail / banner */}
                <div
                  className="project-img-placeholder"
                  style={{
                    background: `linear-gradient(135deg, ${project.tags[0].color}30, ${project.tags[1]?.color || project.tags[0].color}18)`,
                  }}
                >
                  {project.thumbnail ? (
                    <img
                      src={project.thumbnail}
                      alt={project.title}
                      style={{
                        position: "absolute",
                        inset: 0,
                        width: "100%",
                        height: "100%",
                        objectFit: "cover",
                      }}
                    />
                  ) : null}
                  {getProjectIcon(project.tags)}

                  <div className="project-overlay">
                    <div className="overlay-content">
                      <h3>{project.title}</h3>
                      <div className="overlay-buttons">
                        <Link
                          to={`/project/${project.id}`}
                          className="btn-view"
                          onClick={() => window.scrollTo(0, 0)}
                        >
                          View Details
                        </Link>
                        {project.githubUrl && (
                          <a
                            href={project.githubUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="btn-icon"
                            aria-label="GitHub"
                          >
                            <FaGithub />
                          </a>
                        )}
                        {project.liveUrl && (
                          <a
                            href={project.liveUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="btn-icon"
                            aria-label="Live"
                          >
                            <FaExternalLinkAlt />
                          </a>
                        )}
                      </div>
                    </div>
                  </div>
                </div>

                {/* Card body */}
                <div className="project-content">
                  <div className="project-header">
                    <h3>{project.title}</h3>
                    <div className="project-meta">
                      <div className="project-date">
                        <FaCalendarAlt className="meta-icon" />
                        <span>{project.startDate}</span>
                      </div>
                      {project.isOpenSource && (
                        <div className="open-source-tag">
                          <FaCodeBranch className="meta-icon" />
                          <span>OSS</span>
                        </div>
                      )}
                    </div>
                  </div>

                  <p className="project-description">
                    {project.shortDescription}
                  </p>

                  {/* GitHub stats strip (only for OSS with github link) */}
                  {project.githubUrl && project.isOpenSource && (
                    <GitHubStrip githubUrl={project.githubUrl} />
                  )}

                  <div className="project-divider">
                    <div
                      className="project-divider-line"
                      style={{ width: "14%" }}
                    />
                    <span className="project-divider-text">tags</span>
                    <div
                      className="project-divider-line"
                      style={{ width: "100%" }}
                    />
                  </div>

                  <div className="project-tags">
                    {project.tags.map((tag, i) => (
                      <span
                        key={i}
                        className="project-tag"
                        style={{
                          backgroundColor: tag.color,
                          boxShadow: `0 0 10px ${tag.color}55`,
                        }}
                      >
                        {tag.icon} {tag.name}
                      </span>
                    ))}
                  </div>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
        </motion.div>
      </motion.div>
    </section>
  );
};

export default Projects;
